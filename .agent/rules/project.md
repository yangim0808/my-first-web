...
- Ch10 게시글 CRUD: `lib/supabase/client.ts`(Ch8), `useAuth/AuthProvider`(Ch9) 활용. 작성/수정/삭제 등의 기능은 페이지 UX에 집중.
- Ch11 RLS: SQL Editor 직접 실행이 아니라 Supabase CLI 마이그레이션으로 남긴다. `posts` 테이블의 `user_id`와 `auth.uid()`를 기준으로 정책을 만든다. 클라이언트 UI 분기는 보안이 아니며, 실제 보안은 DB의 RLS가 담당한다.
