...
## 4. UI 컴포넌트 구조 및 디자인 토큰
### 4.1. 컴포넌트 계층 (shadcn/ui 기반)
...
Supabase (PostgreSQL)의 두 가지 메인 테이블을 사용합니다.

### 5.1. users 테이블
애플리케이션의 사용자(author) 정보입니다.
- **`id`** (UUID) : Primary Key (Supabase Auth의 User ID와 매핑됨)
- **`email`** (String) : 고유 로그인 및 식별 이메일 (Unique)
- **`name`** (String) : 작성자 표시 이름
- **`created_at`** (Timestamp) : 가입 일시

### 5.2. posts 테이블
작성된 블로그 글 데이터 모델입니다.
- **`id`** (UUID/Integer) : Primary Key (게시글 고유 구별자)
- **`title`** (String) : 게시물 제목
- **`content`** (Text) : 마크다운/HTML 등 본문 데이터
- **`author_id`** (UUID) : **Foreign Key** (참조 -> `users.id`)
- **`created_at`** (Timestamp) : 등록 시간
- **`updated_at`** (Timestamp) : 마지막 반영 시간

### 5.3. 개체 관계 모델링 (Entity Relationships)
- **Users (1) : Posts (N)** 관계 구성.
- 각 포스트는 하나의 특정 Author(`users.id`)에 속합니다. RLS (Row Level Security) 규칙을 통해, 사용자 본인만 자신의 글(`author_id = user_id`)을 수정/삭제할 수 있도록 데이터베이스 차원에서 접근을 제어합니다.
